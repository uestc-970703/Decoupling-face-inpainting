...
# Facial Structure Estimator

This module is the training-stage facial structure estimator for the combined face inpainting framework.

It is adapted from LAP and is loaded by the inpainting loss as a frozen pretrained network. Its outputs are used as online, differentiable structure supervision:
...
- The inpainting Transformer/final output is passed through this estimator, and its `canonical_normal_lap` and `recon_normal_lap` are compared with the estimator outputs from the ground-truth image.
...
The original LAP project description is kept below for reference.
