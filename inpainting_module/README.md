...
This module is the main inference-stage component of the combined face inpainting framework.

It is adapted from MAT and remains responsible for completing masked or missing image regions. During training, it uses `facial_structure_estimator` as a frozen differentiable supervisor:

- Transformer/final output -> `canonical_normal_lap` and `recon_normal_lap` supervision.
- CNN/first-stage output -> `canonical_image_lap` supervision.
- Ground-truth images are passed through the same frozen estimator to form the targets.
...
The original MAT project description is kept below for reference.
